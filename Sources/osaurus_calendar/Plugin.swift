...
private struct CreateEventTool {
    ...
    func run(args: String) -> String {
        ...
        guard let startDate = dateFormatter.date(from: input.startDate),
              let endDate = dateFormatter.date(from: input.endDate) else {
        ...
        case .success(let eventId):
            return "{\"success\": true, \"message\": \"Event \\\"\(escapeJSON(input.title))\\\" created successfully.\", \"eventId\": \"\(escapeJSON(eventId))\"}"
        case .failure(let error):
